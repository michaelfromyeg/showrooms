...
const Editor = () => {
  ...
  const addProducts = () => {
    setOpen(false)
    fetch(
      `https://www.bestbuy.ca/api/v2/json/search?categoryid=&currentRegion=ON&include=facets%2C%20redirects&lang=en-CA&page=1&pageSize=8&path=&query=${productName}%20laptop&exp=&sortBy=relevance&sortDir=desc`
    )
      .then((res) => res.json())
      .then((result) => {
        ...
        return axios
          .patch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/setup/${id}/products`,
            {
              products: {
                description: result.products[0].name,
                location: [x, y],
  ...
  return (
    <div>
      ...
      <div>
        <h3>
          Our machine learning model detected the following items. Please double
          click on the image to add in your products
        </h3>
        {setup
          ? setup.products[0].map((setup) => <li>{setup.description}</li>)
          : 'Detecting....'}
      </div>
      <div>
        <h3>The following items have been added</h3>
        {setup && setup.products.length >= 2
          ? setup.products[1].map((setup) => <li>{setup.description}</li>)
