...
const Product = ({ productSku, isUser }) => {
  ...
  useEffect(() => {
    fetch(
      `https://www.bestbuy.ca/api/v2/json/product/${productSku}?currentRegion=ON&include=all&lang=en-CA`
    )
      .then((res) => res.json())
      .then(
        (result) => {
    ...
    return (
      <>
        ...
        <CardActions>
          {isUser && (
            <Button
              onClick={(e) => handleCopy(e, items.productUrl)}
              variant="contained"
              size="medium"
              color="primary"
            >
